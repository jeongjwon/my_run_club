...
class _AddRecordScreenState extends State<AddRecordScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // String id = (taskProvider.runningsList[taskProvider.runningsList.length] + 1).toString();

    return WillPopScope(
      onWillPop: () async {
        addProvider.resetAllData();
        return true;
      },
      child: Scaffold(
        appBar: AppBar(
                    ...
                    onPressed: () async {
                      ...
                      setState(() {
                        ...
                        totalDistance = addProvider.bigValue.toDouble() +
                            addProvider.smallValue / 100;
                        distanceUnit = addProvider.unit;
                      });
                    },
                    child: Text(
                      !distanceEditing
                          ? '편집'
                          : '${addProvider.bigValue}.${addProvider.smallValue.toString().padLeft(2, '0')} ${addProvider.unit}',
                      style: const TextStyle(color: Color(0xFF8D8D8D)),
                    ),
                  ),
                ],
